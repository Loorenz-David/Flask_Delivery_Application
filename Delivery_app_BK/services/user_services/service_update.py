...
def service_update_user(data: dict, identity=None) -> dict:
    user:User = GetObject.get_object(User, data.get("id"), identity=identity)
    ...
    if 'password' in fields:
        setattr(user, 'password', user.hash_password(fields['password']) )
        del fields['password']


    _update_instance(User, user, fields, relationship_map, identity=identity)
    return {"status": "ok", "instance": user}


def service_update_team(data: dict, identity=None) -> dict:
    team = GetObject.get_object(Team, data.get("id"), identity=identity)
    fields = data.get("fields", {})

    _update_instance(Team, team, fields, relationship_map={}, identity=identity)
    return {"status": "ok", "instance": team}


def service_update_user_role(data: dict, identity=None) -> dict:
    user_role = GetObject.get_object(UserRole, data.get("id"), identity=identity)
    fields = data.get("fields", {})
    relationship_map = {
        "team_id": Team,
        "team": Team,
    ...
    _update_instance(UserRole, user_role, fields, relationship_map, identity=identity)
    return {"status": "ok", "instance": user_role}


def service_update_user_warehouse(data: dict, identity=None) -> dict:
    warehouse = GetObject.get_object(UserWarehouse, data.get("id"), identity=identity)
    fields = data.get("fields", {})
    relationship_map = {
        "team_id": Team,
        "team": Team,
