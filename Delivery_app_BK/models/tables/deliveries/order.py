...
class Order(
    ...
    order_plan_intention = Column(String, index=True)
    ...
    earliest_delivery_date = Column( DateTime(timezone=True), index=True ) 
    latest_delivery_date = Column( DateTime(timezone=True), index=True ) 
    ...
    @validates("order_plan_intention")
    def validate_order_plan_intention(self, key, value):
        ...
        if value not in self.ORDER_PLAN_INTENTIONS:
            raise ValueError(
                f"Invalid order_plan_intention '{value}'. "
